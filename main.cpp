...
void ragebait(const dpp::message_create_t& event)
{
    ...
    const long& current_channel_id = event.msg.channel_id;
    bool do_ragebait = 0;
    for (long poo : ragebaitable_channels)
    {
        if (current_channel_id == poo) do_ragebait = 1;
    }

    if (random != 1) do_ragebait = 0;

    if (!do_ragebait) {std::cout << "message not getting ragebaited" << std::endl; return;}
...
void logMessage(const dpp::message& msg)
{
    ...
    std::cout << "Sent at: " << date_time; // no \n cuz "date_time" already has a new line at its end for some reason
